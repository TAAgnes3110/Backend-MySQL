# TAAgnes Backend - Node.js/Express API với MySQL

**TAAgnes** - Mẫu backend Node.js sẵn sàng cho production sử dụng cơ sở dữ liệu MySQL.

Mẫu thiết kế backend với MySQL của TAAgnes.
...
4. Cấu hình các biến môi trường trong file `.env`:
   - **MySQL**: Cấu hình thông tin kết nối (host, port, database, user, password)
...
```bash
# Khởi động tất cả các dịch vụ (app + database)
docker-compose up -d
...
*Đây là mẫu thiết kế backend với MySQL của TAAgnes.*
